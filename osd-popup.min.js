...
function triggerPopup(triggerType) {
    if (popupTriggered) {
        if (discPopCONFIG.debugMode) console.log('Popup already triggered, ignoring additional triggers');
        return;
    }
    
    if (!shouldShowPopup()) {
        if (discPopCONFIG.debugMode) console.log('Popup blocked by interval restriction');
        return;
    }
    
    // Check if there's already an active timer (safety check)
    const existingTimer = getRemainingTime('popup');
    if (existingTimer !== null && existingTimer > 0) {
        if (discPopCONFIG.debugMode) console.log('Active timer already exists, not creating new one');
        return;
    }
    ...
    // Set timer end time when popup is triggered
...
function initPopupSystem() {
    ...
    // Check if there's an active popup timer (FIRST PRIORITY)
    const popupRemainingTime = getRemainingTime('popup');
    if (popupRemainingTime !== null && popupRemainingTime > 0) {
        // Resume popup display - bypass interval check for active timers
        showCouponPopup();
        startCountdown();
        popupTriggered = true;
        ...
        return; // Exit early - don't set up new triggers
    }

    // Check if there's an active checkout timer (SECOND PRIORITY)
    const checkoutRemainingTime = getRemainingTime('checkout');
    if (checkoutRemainingTime !== null && checkoutRemainingTime > 0) {
        // Resume checkout timer display - user already copied coupon
        isCopied = true;
        showCheckoutTimer();
        
        if (discPopCONFIG.debugMode) {
            console.log('Resumed active checkout timer on page load');
        }
        return; // Exit early - don't set up new triggers
    }

    // ONLY NOW check if popup should be shown based on configured interval
    if (!shouldShowPopup()) {
        if (discPopCONFIG.debugMode) {
            console.log(`Popup not shown - waiting for ${discPopCONFIG.showIntervalDays} day(s) interval`);
        }
        return; // Exit early - don't set up triggers during interval restriction
    }
    
    // Set up all enabled triggers ONLY if no active timers and interval allows
